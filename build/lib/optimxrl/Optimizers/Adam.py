...
class Adam:
    def __init__(
        self, layers=None, beta1=0.9, beta2=0.999, learning_rate=0.01, epsilon=1e-8
    ):
    ...
    def step(self):
        for layer in reversed(self.model_layers):
            if layer.layer_type == "fc" or layer.layer_type == "Conv":
                ...
                layer.m_weights = (
                    self.beta1 * layer.m_weights + (1 - self.beta1) * layer.dweights
                )
                layer.m_bias = (
                    self.beta1 * layer.m_bias + (1 - self.beta1) * layer.dbias
                )

                layer.v_weights = self.beta2 * layer.v_weights + (1 - self.beta2) * (
                    layer.dweights**2
                )
                layer.v_bias = self.beta2 * layer.v_bias + (1 - self.beta2) * (
                    layer.dbias**2
                )

                m_hat_weights = layer.m_weights / (
                    1 - self.beta1 ** (layer.iterations + 1)
                )
                v_hat_weights = layer.v_weights / (
                    1 - self.beta2 ** (layer.iterations + 1)
                )
